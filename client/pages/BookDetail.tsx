...
export default function BookDetail() {
  ...
  const [selectedViewChapterId, setSelectedViewChapterId] = useState<string | null>(null);
  const [editType, setEditType] = useState<"poem" | "book">("book");
  ...
  // Autosave all fields every 20 seconds while editing
  useEffect(() => {
  ...
  const buildChapters = (html: string): { toc: TocItem[]; html: string } => {
    const clean = sanitizeHtml(html || "");
    const template = document.createElement("template");
    template.innerHTML = clean;
    const used = new Set<string>();
    const toc: TocItem[] = [];
    const headings = Array.from(template.content.querySelectorAll("h1,h2,h3")) as HTMLElement[];
    const slug = (t: string) => t.toLowerCase().replace(/[^a-z0-9\s-]/g, "").trim().replace(/\s+/g, "-").slice(0, 80) || "chapter";
    headings.forEach((el, i) => {
      const txt = (el.textContent || "").trim();
      let id = el.id || `ch-${i}-${slug(txt)}`;
      let k = 1;
  ...
  function extractChapterSection(html: string, chapterId: string): { title: string; level: number; content: string } {
    const template = document.createElement("template");
    template.innerHTML = html || "";
    const heading = template.content.querySelector(`#${CSS.escape(chapterId)}`) as HTMLElement | null;
  ...
  function replaceChapterSection(html: string, chapterId: string, newTitle: string, newContent: string): string {
    const template = document.createElement("template");
    template.innerHTML = html || "";
    const heading = template.content.querySelector(`#${CSS.escape(chapterId)}`) as HTMLElement | null;
    ...
    const parent = heading.parentNode as ParentNode;
