...
import { loadPoems, Poem, savePoems, updatePoem, preview as previewPoem, formatDate as formatPoemDate, createPoem, sortPoems, generateId } from "@/lib/poems";
...
export default function Library() {
  ...
  const [typeFilter, setTypeFilter] = useState<"all" | "book" | "poem">("all");
  const [statusFilter, setStatusFilter] = useState<"all" | "draft" | "completed">("all");
  const [sortKey, setSortKey] = useState<"alpha" | "ztoa" | "edited_desc" | "edited_asc">("edited_desc");
  ...
  const filtered = useMemo(() => {
    ...
    let list = poems.filter((p) => {
      ...
      const effectiveType: "book" | "poem" = p.type === "book" || p.type === "poem" ? p.type : "poem";
  ...
  const toggleFavorite = (p: Poem) => setPoems((prev) => prev.map((it) => (it.id === p.id ? { ...it, favorite: !p.favorite } : it)));
      ...
      for (const file of arr) {
        ...
        if (isJSON) {
          try {
            ...
            const imported: Poem[] = Array.isArray(obj) ? obj : Array.isArray((obj as any).poems) ? (obj as any).poems : [];
            imported.forEach((p) => {
              const it: any = { ...p };
  ...
  return (
    <div className="container py-10">
      ...
      <input ref={importRef} type="file" accept=".docx,application/json" multiple className="hidden" onChange={(e) => { const fs = e.target.files; if (fs && fs.length) onImportFiles(fs); e.currentTarget.value = ""; }} />
          ...
          <div className="flex items-center gap-2 rounded-md border bg-background/95 px-3 py-2 shadow-lg w-full sm:w-auto justify-center sm:justify-start">
            ...
            <div className="flex items-center gap-1 text-sm text-muted-foreground">
              ...
              <input
                ...
                onKeyUp={(e) => { if (e.key === "Enter") { const n = Number((e.target as HTMLInputElement).value); if (!Number.isNaN(n)) setPage(Math.min(Math.max(1, n), totalPages)); } }}
